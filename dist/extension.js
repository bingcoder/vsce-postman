...
const nanoid_1 = __webpack_require__(2);
const path = __webpack_require__(5);
const vscode = __webpack_require__(6);
const axios_1 = __webpack_require__(7);
class TreeItemFile {
    constructor(config) {
        this.k = nanoid_1.nanoid();
        this.create_time = +new Date();
        this.ft = vscode.FileType.File;
...
class TreeItemDirectory {
    constructor(config) {
        this.k = nanoid_1.nanoid();
        this.create_time = +new Date();
        this.ft = vscode.FileType.Directory;
        this.env = [];
        this.title = config.title;
        this.pk = config.pk;
    }
...
class PostmanDataProvider {
    ...
    getRecordFromStore() {
        try {
            const records = JSON.parse(fs.readFileSync(this.historyPath).toString() || '[]');
...
class PostmanView {
    constructor(context) {
        ...
        this.panelList = [];
        this.add = (directory) => __awaiter(this, void 0, void 0, function* () {
        ...
        this.deleteItem = (item) => {
            ...
            const key = [];
            (function getChildrenRecursion(root) {
                root.forEach((item) => {
                    key.push(item.k);
                    if (nodeMap[item.k]) {
            ...
            this.treeDataProvider.records = this.treeDataProvider.records.filter((item) => !key.includes(item.k));
            this.treeDataProvider.storage();
            const disposePanels = this.panelList.filter((panel) => key.includes(panel.k));
            disposePanels.forEach((panel) => {
                panel.panel.dispose();
            });
        };
        this.rename = (item) => __awaiter(this, void 0, void 0, function* () {
            const title = yield vscode.window.showInputBox({
                prompt: 'Type the new title',
                value: item.title,
            });
        ...
        this.save = (value) => {
            const index = this.treeDataProvider.records.findIndex((item) => item.k === value.k);
            if (index > -1) {
        ...
        this.saveGroupEnv = (value) => {
            const index = this.treeDataProvider.records.findIndex((item) => item.title === value.title);
            if (index > -1) {
                this.treeDataProvider.records[index].env =
                    value.env;
                vscode.window.showInformationMessage('保存成功');
                this.treeDataProvider.storage();
            }
        ...
        this.receiveMessageHandler = {
            request: axios_1.request,
            saveRecord: this.saveRecord,
            saveGroupEnv: this.saveGroupEnv,
        };
        this.createWebviewPanel = (item) => {
            const panel = vscode.window.createWebviewPanel(extensionKey, item.title, vscode.ViewColumn.Active, {
                enableScripts: true,
                retainContextWhenHidden: true,
            });
            panel.webview.onDidReceiveMessage((message) => {
                var _a, _b;
                (_b = (_a = this.receiveMessageHandler)[message.type]) === null || _b === void 0 ? void 0 : _b.call(_a, message.value, panel);
            });
            const monacoScript = panel.webview.asWebviewUri(vscode.Uri.file(path.join(this.context.extensionPath, 'build/lib/monaco/main.js')));
            const prettierScript = panel.webview.asWebviewUri(vscode.Uri.file(path.join(this.context.extensionPath, 'build/lib/prettier.min.js')));
            const prettierBabelScript = panel.webview.asWebviewUri(vscode.Uri.file(path.join(this.context.extensionPath, 'build/lib/prettier.parser-babel.min.js')));
            const script = panel.webview.asWebviewUri(vscode.Uri.file(path.join(this.context.extensionPath, 'build/index.js')));
            const css = panel.webview.asWebviewUri(vscode.Uri.file(path.join(this.context.extensionPath, 'build/style.css')));
    ...
    <html>
      ...
      <body>
        ...
        this.getPanel = (item) => this.panelList.find((panel) => panel.k === item.k);
        this.handleItemClick = (item) => {
        ...
        vscode.commands.registerCommand('vscPostman.openFile', this.handleItemClick);
    }
    ...
    addFromTitle(ft) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            let pk = null;
            let TreeItemClass = ft === vscode.FileType.Directory ? TreeItemDirectory : TreeItemFile;
            if ((selectItem === null || selectItem === void 0 ? void 0 : selectItem.ft) === vscode.FileType.File) {
                pk = selectItem.pk;
            }
            else if ((selectItem === null || selectItem === void 0 ? void 0 : selectItem.ft) === vscode.FileType.Directory) {
                pk = selectItem.k;
            }
            if (title === null || title === void 0 ? void 0 : title.trim()) {
...
/* harmony export */ });
/* harmony import */ var crypto__WEBPACK_IMPORTED_MODULE_0__ = __webpack_require__(3);
/* harmony import */ var _url_alphabet_index_js__WEBPACK_IMPORTED_MODULE_1__ = __webpack_require__(4);
...
const type_1 = __webpack_require__(46);
const hasBodyMethod = [type_1.Method.POST];
const instance = axios.create();
const requestInfo = {};
function request(message, panel) {
    return __awaiter(this, void 0, void 0, function* () {
        console.log('message', message);
        let startTime = +new Date();
        const { requestId, k } = message, requestConfig = __rest(message, ["requestId", "k"]);
        if (requestInfo[k]) {
        ...
        try {
            const config = Object.assign(Object.assign({ timeout: 30 * 1000 }, requestConfig), { validateStatus(status) {
                    return status >= 200 && status <= 599;
                } });
            const res = yield instance.request(config);
            console.log('res', res);
            value = Object.assign(Object.assign({}, value), res);
        }
        catch (error) {
            value = Object.assign(Object.assign({}, value), { data: error.message, statusText: 'Error' });
        }
        finally {
            if (!panel || requestId !== requestInfo[k].currentRequestId)
                return;
            panel.webview.postMessage({
                source: 'vsce-postman',
                type: 'response',
                value: Object.assign(Object.assign({}, value), { spend: +new Date() - startTime }),
            });
        }
    });
}
...
Object.defineProperty(exports, "__esModule", ({ value: true }));
exports.Method = exports.ContentType = void 0;
var ContentType;
(function (ContentType) {
    ContentType[ContentType["JSON"] = 1] = "JSON";
    ContentType[ContentType["FormData"] = 2] = "FormData";
    ContentType[ContentType["UrlEncoded"] = 3] = "UrlEncoded";
